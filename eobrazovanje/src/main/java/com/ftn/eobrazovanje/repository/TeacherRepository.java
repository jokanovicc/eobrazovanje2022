...
public interface TeacherRepository extends JpaRepository<Teacher, Long> {
    ...
    @Query(value = "select * from teacher t where t.user_id not in (select ct.teacher_user_id from course_teacher ct where ct.active = true and ct.id in (select p.teacher_id from performance_teacher_relationship p where p.performance_id = ?1))", nativeQuery = true)
